...
# def normalization (base_signal):
#       return (base_signal - base_signal.mean()) / base_signal.std()
...
def Gaussian(base_signal, smoothing_sigma=1.0):
    ...
    # data_abs_max = np.max(data_abs)
    data_mean = np.mean(base_signal)
    data_variance = np.var(base_signal,ddof=1)
    data_std = np.std(base_signal)
    # norm_func = lambda x: (x - data_abs_min) / (data_abs_max - data_abs_min)
    # a = (1/(math.sqrt(2*math.pi*data_variance)))
    # b = ((base_signal - data_mean)*(base_signal - data_mean))
    # data_norm = a*np.exp (-b)/ (2*data_variance)
...
def decompose(data):
    ...
    for trial in range(24):
        ...
        for channel in range(14):
            ...
            base_signal = Gaussian(base_signal, smoothing_sigma=1.0)
...
if __name__ == '__main__':
    ...
    for file in sorted(glob.glob(dataset_dir+'*.mat')):
        ...
        data = read_file(file)
        base_DE, trial_DE, data_seconds_list = decompose(data['joined_data1'].copy())
